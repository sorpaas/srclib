...
func testTree(treeDir, expectedDir, actualDir string, generate bool) error {
	...
	origOutputDestDir := filepath.Join(treeDir, buildstore.BuildDataDirName, "TEST")
	...
	cmd := exec.Command("src", "-v", "make", "-m", "docker", "--subdir", ".", "--repo", treeName, "--repo-root", ".", "--commit-id", "TEST")
	cmd.Dir = treeDir
